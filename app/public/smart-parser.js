// ============================================================
//  PONTAJ MANAGER - SMART PARSER (AI EXTRACTOR FOR EXCEL & PDF)
// ============================================================

setTimeout(() => {
    const sel = document.getElementById('smart-import-proj');
    if (sel && typeof _projects !== 'undefined') {
        sel.innerHTML = '<option value="">-- Detectare Automată (în funcție de SMIS) --</option>';
        _projects.forEach(p => {
            sel.innerHTML += `<option value="${p.id}">${p.name} (SMIS ${p.smis})</option>`;
        });
    }
}, 1500);
...
function groupPdfItemsIntoRows(items) {
    ...
    // Sortam de sus in jos (Y descrescator = de la top pagina)
    const sortedItems = [...items].sort((a, b) => b.transform[5] - a.transform[5]);

    for (const item of sortedItems) {
        const y = Math.round(item.transform[5]);
        // Toleranta MICA (2px) → rânduri adiacente în tabel nu se mai amestecă
...
window.handleSmartImport = async function (event) {
    const files = event.target.files;
    if (!files || !files.length) return;

    const logs = document.getElementById('smart-import-logs');
    logs.innerHTML = `<i>Începe procesarea pentru ${files.length} fișier(e)...</i><br>`;

    const log = (msg) => {
        logs.innerHTML += `<div style="margin: 2px 0;">${msg}</div>`;
        logs.scrollTop = logs.scrollHeight;
    };

    if (typeof _projects === 'undefined' || typeof _persons === 'undefined' || !_projects.length) {
        log('<span style="color:var(--red)">❌ Eroare: Nu sunt proiecte sau persoane în baza de date!</span>');
        return;
    }

    const forcedProjectId = document.getElementById('smart-import-proj')?.value;
    let forcedProject = null;
    if (forcedProjectId) {
        forcedProject = _projects.find(p => p.id === forcedProjectId);
        log(`<span style="color:var(--accent2)">ℹ️ Mod de import forțat pentru proiectul: <b>${forcedProject.name} (SMIS ${forcedProject.smis})</b></span>`);
    }
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        log(`<br><span style="color:#fff; border-top: 1px solid #333; display: block; padding-top: 8px;"><b>📄 [${file.name}]</b> Analiză...</span>`);
        ...
        try {
            ...
            if (!foundPersonId) {
                log(`<span style="color:var(--text-muted)">• Ignorat: Nu am găsit numele niciunei persoane cunoscute.</span>`);
                continue;
            }
            if (!foundYear || !foundMonth) {
                log(`<span style="color:var(--text-muted)">• Ignorat (${foundPersonName}): Nu am putut detecta luna (${foundMonth})/anul (${foundYear}).</span>`);
                continue;
            }
            if (!foundSmis) {
                log(`<span style="color:var(--text-muted)">• Ignorat (${foundPersonName}): Nu am găsit cod SMIS, te rog selectează manual Proiectul Țintă.</span>`);
                continue;
            }

            const proj = forcedProject || _projects.find(px => String(px.smis) === String(foundSmis));
            if (!proj) {
                log(`<span style="color:var(--gold)">⚠️ Lipsă proiect: SMIS ${foundSmis} nu e creat!</span>`);
                continue;
            }
            ...
            for (const sheetName in sheetsRows) {
                ...
                if (sheetName.startsWith('PAGE_')) {
                    ...
                    if (!pageHasAnyInterval) {
                        ...
                        log(`<span style="color:#6a7a96;font-size:11px">  ⏭ ${sheetName}: fara intervale detectate → tratat ca pagina RA (ore ignorate)</span>`);
                    }
                }
                ...
                if (!isPdf && foundSmis) {
                    ...
                    if (intervalCols.length >= 2 && proj) {
                        ...
                        if (isLaserFo) {
                            ...
                            log(`<span style="color:var(--text-muted);font-size:11px">🔍 Tabel Multi-Proiect: Extrag doar ${proj.name} din stânga (col ${colFilterStart} la ${colFilterEnd - 1}).</span>`);
                        } else if (isAirNavy) {
                            colFilterStart = Math.max(1, intervalCols[1] - 1);
                            colFilterEnd = 999;
                            targetCol = -2; // Stop normal logic
                            log(`<span style="color:var(--text-muted);font-size:11px">🔍 Tabel Multi-Proiect: Extrag doar ${proj.name} din dreapta (col ${colFilterStart} la final).</span>`);
                        }
                    }
                    ...
                    if (targetCol === -1) {
                        ...
                        if (targetCol !== -1) {
                            ...
                            colFilterStart = 0;
                            colFilterEnd = 999;
                            log(`<span style="color:var(--text-muted);font-size:11px">🔍 Avertisment: Nu am găsit codul ${foundSmis} pe nicio coloană. Citesc orele automat.</span>`);
                        }
                    }
                }
                        ...
                        if (dayHours !== null) {
                            ...
                            if (!finalDays.hasOwnProperty(dayNum)) {
                                ...
                                if (typeof dayHours === 'number') {
                                    log(`<span style="color:#6a7a96;font-size:11px">  → Zi ${dayNum}: ${dayHours}h (${sheetName})</span>`);
                                }
                            }
                        }
            ...
            if (confirmedHours.length >= 3) { // avem suficiente zile confirmate pentru a calcula modul
                ...
                Object.keys(finalDays).forEach(d => {
                    ...
                    if (typeof h === 'number' && !finalIntervals[d]) {
                        ...
                        if (Math.abs(h - modeHours) > modeHours * 0.5) {
                            log(`<span style="color:var(--gold);font-size:11px">⚠️ Zi ${d}: ${h}h (fara interval, deviere fata de mod ${modeHours}h) → corectat la ${modeHours}h</span>`);
                ...
                if (correctedCount > 0) {
                    log(`<span style="color:var(--gold)">🔁 Cross-validare: ${correctedCount} zi(le) corectate automat (mod = ${modeHours}h/zi)</span>`);
                }
            }
            ...
            if (Object.keys(finalDays).length === 0 && Object.keys(finalActivities).length === 0) {
                log(`<span style="color:var(--gold)">⚠️ N-am extras nimic pentru ${foundPersonName}. Poate formatul intern e prea modificat.</span>`);
                continue;
            }
            ...
            log(`<span style="color:var(--green)">✅ <b>[SMIS ${proj.smis}] ${MONTH_DICT[foundMonth - 1]} ${foundYear}</b> pt <b>${foundPersonName}</b>. Găsit: ${totalHoursFound}h pontate, ${intCnt} intervale, ${txtCnt} descrieri.</span>`);

            try {
                let mData = {};
                try {
                    const existingRes = await fetch(`http://localhost:3000/api/pontaj/${proj.id}/${foundYear}/${foundMonth}`);
                    if (existingRes.ok) mData = await existingRes.json();
                } catch (e) { }
                ...
                try {
                    ...
                    if (projFresh && !projFresh.members.find(m => m.personId === foundPersonId)) {
                        ...
                        await window.apiPost('/projects', projFresh);
                    }
                } catch (e) { /* silent - nu blocam importul pt o eroare la membri */ }

                await window.apiPost(`/pontaj/${proj.id}/${foundYear}/${foundMonth}`, mData);
                successCount++;
            } catch (e) {
                log(`<span style="color:var(--red)">Eroare rețea salvard: ${e.message}</span>`);
            }

        } catch (err) {
            log(`<span style="color:var(--red)">Eroare la citirea fișierului ${file.name}: ${err.message}</span>`);
        }
    }

    log(`<br><span style="color:var(--accent2);font-weight:700">🚀 IMPORT COMPLET! Baza de date este actualizată.</span>`);
    log(`<span style="color:var(--text-muted)">Au fost înregistrate cu succes ${successCount} seturi de date.</span>`);
    event.target.value = '';
